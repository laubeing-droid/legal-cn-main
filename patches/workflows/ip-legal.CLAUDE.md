...
在处理本领域任何任务前，必须先加载 `references/reasoning-template-zh.md` 中的中国法推理链。
该模板定义了本领域在中国法下的正确推理结构，替代美国法推理模式。

核心纪律：
- 使用三段论（法律规范→案件事实→裁判结论），禁止使用 IRAC
- 民商事领域：使用请求权基础分析法（请求权目标→请求权基础→构成要件检视→法律效果）
- 行政法规领域：使用行政行为合法性审查（职权→事实→程序→裁量）
- 引用法条必须标注具体条目，禁止笼统引用法律名称


## 中国法适配护栏（强制性）

以下规则具有最高优先级，优先于本技能其它指令：

### 规则 1 — 美国法概念阻断
当用户提及以下美国法概念时，必须立即指出中国法无此制度，并给出中国法替代方案：

| 美国法概念 | 中国法处理 |
|:----------|:----------|
| deposition（庭前宣誓证言） | 阻断 + 替换为"调查取证准备"（调查令/证据保全/质证） |
| consideration（对价） | 阻断，中国合同成立不以对价为要件 |
| at-will employment（自由雇佣） | 阻断，中国劳动法实行法定解雇限制制 |
| discovery（证据开示） | 阻断 + 替换为"证据交换与法院调查取证程序" |
| privilege log | 阻断，中国无 attorney-client privilege 制度 |
| cease and desist | 替换为"律师函" |
| severance package | 替换为"经济补偿金"或"赔偿金" |
| precedent / 判例法推理 | 阻断，中国为成文法，判例仅具参考价值 |
| plea bargaining | 替换为"认罪认罚从宽" |
| false light | 替换为"侵害名誉权" |
| hearsay rule / Miranda / habeas corpus | 阻断，中国无对应制度 |
| summary judgment | 阻断，中国无即决判决制度 |
| statute of frauds | 阻断，中国无此制度 |

### 规则 2 — 禁止跨境法域引用
- Delaware 公司法 / 美国州法 → 阻断跨境引用，适用中国公司法
- layoff / RIF（裁员）→ 替换为中国经济性裁员程序（劳动合同法第41条）
不得主动引用美国、英国、香港等法域的法律依据。当用户提及境外法域时，必须标注法域差异并优先适用中国法。

### 规则 3 — 阻断必替代
凡触发阻断概念，必须在同一回答中提供中国法等效制度或实务指引。

### 规则 4 — 输出自检
最终输出前检查是否包含阻断清单中的概念，如有则修正或删除。
## 公司画像
