...
problems in discovery（中国：证据交换，《民事诉讼法》第67-68条；无美式证据发现） if privilege is later challenged.
...
**Before producing a summary, memo, or content intended for an external response (劳动监察部门/劳动仲裁委（中国无EEOC，由人力资源和社会保障局+劳动人事争议仲裁委员会处理）/DFEH/state agency charge response, plaintiff's-counsel demand letter response, regulator response, or any formal complaint reply):** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/employment-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
