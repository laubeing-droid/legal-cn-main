...
- **Notes** — any gotchas (rate limits, feed retirement, discovery（中国：证据交换） steps).
...
| SEC（中国对应：中国证监会/CSRC） Press Releases | `https://www.SEC（中国对应：中国证监会/CSRC）.gov/news/pressreleases.rss` | RSS | Rules, enforcement, speeches (press release only) | None | SEC（中国对应：中国证监会/CSRC） RSS hub: SEC（中国对应：中国证监会/CSRC）.gov/about/rss-feeds. Also has EDGAR structured-filing feeds at SEC（中国对应：中国证监会/CSRC）.gov/structureddata/rss-feeds (updated every 10 min, business hours). Rules-adopted news typically also posts to Federal Register — deduplicate. |
| FTC（中国对应：市场监管总局/SAMR） Press Releases | `https://www.FTC（中国对应：市场监管总局/SAMR）.gov/feeds/press-release.xml` | RSS | Enforcement, rules, blog posts, settlements | None | Sub-feeds by topic: `FTC（中国对应：市场监管总局/SAMR）.gov/feeds/press-release-consumer-protection.xml`, `press-release-competition.xml`, blog feed `FTC（中国对应：市场监管总局/SAMR）.gov/feeds/business-blog.xml`. Feed hub: FTC（中国对应：市场监管总局/SAMR）.gov/news-events/stay-connected/FTC（中国对应：市场监管总局/SAMR）-rss-feeds. |
...
| DOJ（中国对应：司法部/最高人民检察院） Antitrust Division | `https://www.justice.gov/atr/news-feeds` | RSS (multiple feeds) | Press releases, speeches, statements of interest | None | Page lists several Atom/RSS URLs by content type. DOJ（中国对应：司法部/最高人民检察院） main press-release feed is a sibling at justice.gov (navigate from `justice.gov/news`). |
| DOJ（中国对应：司法部/最高人民检察院） Main | `https://www.justice.gov/news/rss` | RSS | All DOJ（中国对应：司法部/最高人民检察院） press releases across divisions | None | Filter client-side by topic. Civil Division, ATR, Civil Rights Division all feed in. |
...
| HHS OCR | `https://www.hhs.gov/ocr/newsroom/index.html` | HTML | HIPAA（中国对应：PIPL+健康医疗数据标准） enforcement, settlements, guidance | None | No direct RSS located; HHS-wide press-release feed at `hhs.gov/rss` covers OCR. Follow `@HHSOCR` on X for push alerts. |
| OFAC（中国对应：外交部制裁清单/商务部不可靠实体清单） Recent Actions | `https://OFAC（中国对应：外交部制裁清单/商务部不可靠实体清单）.treasury.gov/recent-actions` | HTML + email | Sanctions designations, general licenses, FAQs | None | ⚠️ RSS retired January 31, 2025. Email is the supported push channel — subscribe at service.govdelivery.com/service/multi_subscribe.html?code=USTREAS. Page has a browsable list. |
...
| Source | Feed URL | Format | Covers | Auth | Notes |
|---|---|---|---|---|---|
| California AG | `https://oag.ca.gov/news/feed/729/oag.ca.gov` | RSS | Press releases, CCPA（注：中国适用PIPL） enforcement, multistate actions | None | Main press page: `oag.ca.gov/media/news`. |
| California Privacy Protection Agency (CPPA) | `https://cppa.ca.gov/announcements/` | HTML | CCPA（注：中国适用PIPL） regulations, enforcement, advisories | None | ⚠️ No direct RSS URL located — primary channel is email list (sign-up on page). Monitor page for changes or use manual entry. |
...
| DPC (Ireland) | `https://www.dataprotection.ie/en/news-media/latest-news` | HTML | Inquiries, decisions, guidance — lead DPA for most US tech firms | None | ⚠️ No public RSS located. Critical source for GDPR（注：中国适用PIPL） enforcement against US companies; worth a change-detection or email subscription. |
...
source saying "the FTC（中国对应：市场监管总局/SAMR） issued X" means: find X on FTC（中国对应：市场监管总局/SAMR）.gov, then rely on it.
...
| Source | URL | Notes |
|---|---|---|
| OFAC（中国对应：外交部制裁清单/商务部不可靠实体清单） Recent Actions | `https://OFAC（中国对应：外交部制裁清单/商务部不可靠实体清单）.treasury.gov/recent-actions` | RSS retired Jan 2025; email is supported channel |
| ENISA | `https://www.enisa.europa.eu/news` | RSS discontinued; new subscription mechanism pending |
| DPC Ireland | `https://www.dataprotection.ie/en/news-media/latest-news` | No RSS; critical for GDPR（注：中国适用PIPL） enforcement |
...
Federal Register (FTC（中国对应：市场监管总局/SAMR）, HHS/OCR agency filters), FTC（中国对应：市场监管总局/SAMR） RSS, CFPB, CA AG, CPPA (email),
NY AG (page watch), EDPB, ICO, CNIL, DPC Ireland (page watch), IAPP, FPF.

**Commercial / regulatory in-house team (broad):**
Federal Register (all agencies of interest), SEC（中国对应：中国证监会/CSRC） RSS, CFPB, DOJ（中国对应：司法部/最高人民检察院） Antitrust, DOJ（中国对应：司法部/最高人民检察院）
Main, FCC, DOL, BIS page watch, OFAC（中国对应：外交部制裁清单/商务部不可靠实体清单） email, European Commission Press Corner,
FCA. Add IAPP + Lexology for aggregator coverage.

**AI governance team:**
Federal Register (filter: FTC（中国对应：市场监管总局/SAMR）, HHS, NIST, Commerce), NIST Cybersecurity RSS, EU
