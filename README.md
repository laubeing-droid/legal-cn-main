...
本仓库的 MCP 配置委托给独立仓库 [Codex-Claude-legal-cn-mcp-hub](https://github.com/laubeing-droid/Codex-Claude-legal-cn-mcp-hub) 管理，`install.ps1` 和 `update.ps1` 自动调用。支持三种连接方式：

| 连接器 | 方式 | 工具数 | 凭证 | 推荐 |
|--------|------|--------|------|------|
| **元典-法律法规** | HTTP Streamable | 5 | Bearer API Key | ? 首选 |
| **元典-案例文书** | HTTP Streamable | 4 | Bearer API Key | ? 首选 |
| **元典-企业信息** | HTTP Streamable | 26 | Bearer API Key | ? 首选 |
| **元典（npm备选）** | stdio (npx) | 33 | CHINESELAW_API_KEY | 备选 |
| **元典（REST API）** | HTTP 直调 | 36 | X-API-Key | 灵活集成 |
| **飞书 LarkSuite** | stdio (npx) | 文档/消息/日历 | App ID + Secret | 推荐 |
| **北大法宝 HTTP** | HTTP（10 个服务） | 10 | Bearer Access Token | 推荐 |
| **国家法律法规数据库** | Python 自建 | 11 | 无需认证 | 免费备选 |
| **人民法院案例库** | Python 自建 | 7 | Cookie Token | 需 Token |
...
~/.codex/skills/<domain>/             ← 运行层
  SKILL.md + CLAUDE.md + references
       │
       ▼
~/.codex/config.toml                  ← MCP 层（由 mcp-hub 仓库管理）
...
| npm 包（@pkulaw/mcp-cli） | 检测新版本 -> 创建 Issue |
